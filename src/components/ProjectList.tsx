      ...
      <div className="p-6 border-b-4 border-pixel-border">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          ...
          <div className="w-full sm:w-64">
            <label className="block text-xs text-pixel-text-muted mb-2 uppercase tracking-wider">Search</label>
            <input
              type="text"
              placeholder="Find projects..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pixel-input text-sm placeholder-pixel-text-muted"
            />
          </div>
        </div>
      </div>
      ...
      <div className="divide-y-4 divide-pixel-border">
        {filteredAndSortedProjects.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-pixel-text-muted text-sm">Try adjusting your search or filters</p>
          </div>
        ) : (
          filteredAndSortedProjects.map((project) => (
            <div
              ...
              className={`group p-6 cursor-pointer transition-all duration-100 ${
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4 min-w-0" style={{ width: 'calc(100% - 576px)' }}>
                  ...
                  <div className="flex flex-col min-w-0">
                    <h3 className="text-base font-pixel text-pixel-text group-hover:text-pixel-accent truncate">
                      {project.title}
                    </h3>
                    <span className="text-sm text-pixel-text-muted mt-1 truncate">{project.repository_url.split('/').pop()}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between flex-shrink-0 space-x-6" style={{ width: '576px' }}>
                  <div className="w-36 text-right">
                    <div className="text-xs text-pixel-text-muted mb-1 text-right">REWARD</div>
                    <div className="font-pixel text-sm text-pixel-accent text-right flex items-center justify-end space-x-2">
                  ...
                  <div className="w-44 text-right">
                    <div className="text-xs text-pixel-text-muted mb-1 text-right">START</div>
                    <div className="text-base text-pixel-text text-right">
                      {project.start_date
                  ...
                  <div className="w-44 text-right">
                    <div className="text-xs text-pixel-text-muted mb-1 text-right">END</div>
                    <div className="text-base text-pixel-text text-right">
                      {project.end_date
